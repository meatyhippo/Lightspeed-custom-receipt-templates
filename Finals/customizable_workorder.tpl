	...
	{% for Workorder in Workorders %}
		<div class="workorder {% if not loop.last %} pagebreak{% endif %}">
			...
			<div class="header">
				...
				{% endif %}
				{% if minimal_mode == true %}
					<h1 id="receiptTypeTitle">Herstelling/Repair #{{ Workorder.workorderID }}</h1>
				...
				{% else %}
						...
						{% elseif make_work_order_number_small == false %}	
							<h1 id="receiptTypeTitle">Herstelling/Repair</h1>
							{{ _self.date(Workorder) }}
							<br />
        ...
        {% endif %}
				...
				{% for serializedID in Workorder.Serialized %}
					{% if hide_work_order_item == false %}
						...
						<div class="workOrderItem" >Herstelling/Repair:</div>
				...
				{% if hide_start_and_date == false %}
					Started: {{Workorder.timeIn|correcttimezone|date ("d/m/y")}}<br />
					{% else %}
				{% endif %}
</br>
				{% if hide_employee == false %}
					<p style="margin-left: 1px;">Employee: {{ Workorder.Employee.firstName }} {{ Workorder.Employee.lastName }}</p>
				{% else %}
				{% endif %}
				</h2>
			</div>

			<table class="lines hidetotals">
...
{% macro date(Workorder) %}
    <p>
        {{"now"|correcttimezone|date('d/m/Y')}}
    </p>
{% endmacro %}
